...
## 1. Overview
We use **PostgreSQL** driven by **Prisma ORM**. The data architecture is designed for **Multi-Tenancy** and **Horizontal Scalability**.
...
- **Schema:** `frameworks/next/lib/app-infrastructure/database/schema.ts` (Prisma/Zod definition source).
- **Client:** `frameworks/next/lib/app-infrastructure/database/index.ts` (Singleton PrismaClient).
- **Migrations:** `frameworks/next/lib/app-infrastructure/database/migrations/` (SQL history).
- **Seeds:** `frameworks/next/prisma/seed.ts` (Initial data population).
...
- **Querying:** `db.quiz.findMany({ where: { workspaceId: ctx.workspaceId } })`
- **Indexing:** Indexes should almost always be compound: `@@index([workspaceId, status])`.
...
- **ALWAYS** respect the unique constraints and foreign keys defined in `schema.prisma`.
...
- **DO** be mindful of "N+1" query problems. Use `include: { ... }` in Prisma to fetch relations efficiently.
