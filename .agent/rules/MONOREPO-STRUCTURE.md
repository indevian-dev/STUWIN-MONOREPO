...
- **Domains:** (e.g., `learning`, `auth`, `workspace`)
- **Services:** Business logic (e.g., `LearningService`).
- **Repositories:** Database access (e.g., `LearningRepository`).
...
- **ALWAYS** place database queries in *Repositories*. Services should rely on Repositories, not `prisma` directly.
