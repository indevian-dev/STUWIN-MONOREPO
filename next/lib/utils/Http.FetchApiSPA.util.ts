...
import type { ApiError } from '@stuwin/shared/types/api/ApiResponse.types';
...
export async function retryAfter2FA(originalRequest: FetchOptions): Promise<AxiosResponse> {
  ...
  return fetchApi({ method, url, params, body, headers });
}

/**
 * Typed API call wrapper that unwraps the standard response envelope.
 * Usage: const subjects =await fetchApiUtil<Subject[]>({ method: 'GET', url: '/api/...' });
 */
export async function fetchApiUtil<T>(options: FetchOptions): Promise<T> {
