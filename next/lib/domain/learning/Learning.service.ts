...
export class LearningService extends BaseService {
    ...
    async getSubjectById(subjectId: string) {
        try {
            ...
            // displayTitle is just the raw name now, as we don't force suffixes
            const processed = {
                ...subject,
                title: subject.name || '',
     ...
     */
    async updateSubject(id: string, data: Partial<SubjectCreateInput> & {
        ...
        try {
            ...
            if (subjectName) {
                // No regex validation. Directly update name.
     ...
     */
    async createTopicWithContent(subjectId: string, data: TopicCreateInput & { pdfFileName?: string; pdfId?: string }) {
        try {
     ...
     */
    mapQuestionToLegacy(data: QuestionProviderView) {
        const q = data;
        return {
            id: q.id,
            body: q.question,
     ...
     * Update a topic
     */
    async updateTopic(topicId: string, data: Partial<TopicCreateInput>) {
        try {
     ...
     * Bulk create topics
     */
    async bulkCreateTopics(subjectId: string, topicsData: (Omit<TopicCreateInput, 'providerSubjectId'> & { pdfFileName?: string, pdfPagesByTopic?: number[], pdfPageStart?: number, pdfPageEnd?: number, pdfS3Key?: string, body?: string })[]) {
        try {
            const result = await this.db.transaction(async (tx) => {
                ...
                const createdTopics = await this.repository.bulkCreateTopics(topicsToInsert, tx);
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...
                    questions: result.data.questions as unknown as QuestionProviderView[],
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...
                    fullS3Key: s3Key, // In case client needs it directly
    ...
    async saveTopicPdfMetadata(topicId: string, data: { fileName: string, pdfPageStart?: number, pdfPageEnd?: number, chapterNumber?: string }) {
        try {
            ...
            try {
                const { getTotalPages } = await import("@/lib/utils/Extractor.Pdf.util");
                const s3Prefix = process.env.NEXT_PUBLIC_S3_PREFIX || "";

                // Construct deterministic S3 Key
                const s3Key = `topics/pdfs/${topicId}/${data.fileName}`;
  ...
  "topics": [
    {
      ...
      "pageEnd": 10
            ...
            const validTopics = parsedResponse.topics
                ...
                .map((t: { name: string, pageStart: number, pageEnd: number }) => ({
                    name: t.name.trim(),
                    pageStart: t.pageStart,
                    pageEnd: t.pageEnd,
                } as { name: string, pageStart: number, pageEnd: number }));
