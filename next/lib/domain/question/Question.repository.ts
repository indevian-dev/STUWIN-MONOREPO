...
export class QuestionRepository extends BaseRepository {
    ...
    async list(params: {
        ...
        if (params.onlyUnpublished) filters.push(eq(providerQuestions.isPublished, false));
    ...
    async count(params: {
        ...
        authorAccountId?: string; onlyPublished?: boolean; onlyUnpublished?: boolean; workspaceId?: string;
    }, tx?: DbClient) {
        const client = tx ?? this.db;
        const filters = [];
        if (params.providerSubjectId) filters.push(eq(providerQuestions.providerSubjectId, params.providerSubjectId));
