...
export function AuthVerificationWidget({
    ...
    const t = useTranslations('AuthVerificationWidget');
    ...
    return (
        <div className="w-full flex justify-center items-center min-h-screen px-4">
            <div className="w-full max-w-md">
                <form onSubmit={handleVerify} className="bg-white rounded-app-primary p-4 sm:p-8 space-y-6">
                    {showLogo && (
                        <Link href="/" className={`inline-flex flex-col ${isEmail ? 'items-start' : 'items-center'} gap-2 text-app-dark-blue dark:text-white hover:text-app-bright-green transition-colors`}>
                            <GlobalLogoTile width={200} height={50} />
                            <span className="text-xs text-app-dark-blue/70 dark:text-white/70">{t('back_to_home')}</span>
                        </Link>
                    )}

                    <div className={`space-y-2 ${isEmail ? 'text-left' : 'text-center'}`}>
                        <h1 className="text-3xl font-bold text-app-dark-blue dark:text-white">{texts.title}</h1>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1">
                            <label className="block text-sm font-medium text-app-dark-blue dark:text-white" htmlFor="target">
                                {texts.label}
                            </label>
                            <input
                                id="target"
                                ...
                                className="w-full rounded-app bg-black/5 dark:bg-white/5 backdrop-blur-md py-3 px-3 text-app-dark-blue dark:text-white focus:outline-none focus:ring-1 focus:ring-app-soft"
                                value={targetState}
                        ...
                        <div className="space-y-1">
                            <label className="block text-sm font-medium text-app-dark-blue dark:text-white" htmlFor="otp">
                                {t('verification_code')}
                            </label>
                            <input
                                className="w-full rounded-app bg-white py-3 px-3 text-app-dark-blue dark:text-white focus:outline-none focus:ring-1 focus:ring-app-soft text-center text-2xl font-mono tracking-widest transition-all"
                                id="otp"
                                name="otp"
                                type="text"
                                placeholder="000000"
                    ...
                    <div className="space-y-3 pt-2">
                        <button
                            className="w-full rounded-app-primary bg-app-bright-green-soft hover:bg-app-bright-green/90 text-white font-semibold py-3 px-4 transition disabled:opacity-50 disabled:cursor-not-allowed"
                            type="submit"
                        ...
                        <button
                            ...
                            className="w-full rounded-app-primary bg-black/5 dark:bg-white/5 backdrop-blur-md hover:bg-app-bright-green-border text-app-dark-blue dark:text-white font-semibold py-3 px-4 transition disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={isSending}
                        >
                        ...
                        {showBackLink && (
                            <button
                                ...
                                className="w-full text-app-bright-green hover:text-app-bright-green/80 text-sm font-medium py-2 transition"
                            >
                                {texts.backText}
                            </button>
                        )}
