  ...
  return (
    <section className="max-w-screen-xl m-auto px-4 my-4 md:my-6 lg:my-8">
      <div className='grid grid-rows-2 grid-flow-col gap-3 overflow-x-scroll py-2 pb-4 text-sm'>
        <Link key={'map'} href={`/map`} passHref className={`col-span-6 lg:col-span-8 px-2 pt-2 pb-10 rounded flex items-center  relative bg-app-bright-green text-app-bright-green-light`} >
        ...
        {fastNavigationSubjects && fastNavigationSubjects.length > 0 && fastNavigationSubjects.map((subject, index) => {
          return (
            <Link key={subject.id} href={`/subjects/${subject.slug}-${subject.id}`} passHref className={`col-span-12 px-2 pt-2 pb-4 rounded flex items-start  relative overflow-hidden ${subject.type === 'digital' ? 'bg-app-bright-green-dark/10 text-app-dark-blue dark:text-white' : 'bg-app-bright-green/5 text-app-dark-blue dark:text-white'}`} >
              {subject.icon ? (
