                ...
                <Link key={blog.id} href={'/blogs/' + blog.slug + '-' + blog.id} className='p-5 col-span-6 w-full'>
                    <div className="w-full grid grid-cols-12">
                        <div className="relative flex col-span-12 flex-col w-full p-20 justify-center items-start px-6 tracking-wide">
                            <Image className='rounded-app' style={{ objectFit: 'cover' }} src={blog?.cover ? `${process.env.NEXT_PUBLIC_BLOG_COVER_URL_PREFIX + '/' + blog?.id + '/' + blog.cover}` : '/pg.webp'} fill alt={blog?.title?.content ? blog.title?.content : 'Blog title'} />
                        </div>
