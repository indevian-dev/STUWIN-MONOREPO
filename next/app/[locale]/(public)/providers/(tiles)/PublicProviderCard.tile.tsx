...
export function PublicProviderCardTile({ Provider, compact = false }: PublicProviderCardTileProps) {
  ...
  if (compact) {
    return (
      <Link
        ...
        className="flex items-center gap-3 bg-white border border-gray-200 rounded-app p-3 hover:shadow-md transition-all group"
      >
        <div className="relative w-16 h-16 flex-shrink-0 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-app overflow-hidden">
          <Image
            src={logoUrl}
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-sm text-gray-900 truncate group-hover:text-app-bright-green transition-colors">
            {Provider.title}
          </h3>
          <div className="flex items-center gap-2 mt-1">
            ...
            {Provider.providerSubscriptionPrice !== undefined && (
              <p className="text-xs font-bold text-app-bright-green ml-auto">
  ...
  return (
    <Link
      href={`/providers/${Provider.id}`}
      className="block bg-white border border-gray-200 rounded-app overflow-hidden hover:shadow-lg transition-all group"
    >
      ...
      <div className="p-3">
        <h3 className="font-medium text-sm text-gray-900 line-clamp-1 group-hover:text-app-bright-green transition-colors">
          {Provider.title}
        </h3>
        <div className="flex items-center justify-between mt-1">
          ...
          {Provider.providerSubscriptionPrice !== undefined && (
            <p className="text-xs font-bold text-app-bright-green">
              {Provider.providerSubscriptionPrice === 0 ? 'Free' : `${Provider.providerSubscriptionPrice} AZN`}
            </p>
          )}
        </div>
