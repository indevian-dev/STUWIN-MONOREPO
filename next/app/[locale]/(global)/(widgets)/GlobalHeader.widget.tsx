'use client';

import React from 'react';
import { DomainNavConfig } from '@stuwin/shared/types/ui/Navigation.types';
import { GlobalLogoTile } from '@/app/[locale]/(global)/(tiles)/GlobalLogo.tile';

interface GlobalHeaderWidgetProps {
  config: DomainNavConfig;
  isMenuOpen: boolean;
  setIsMenuOpen: (isOpen: boolean) => void;
  children?: React.ReactNode;
}

/**
 * Global Header Widget
 * Provides consistent header structure across all domains
 * with domain-specific branding and menu toggle
 */
export function GlobalHeaderWidget({
  config,
  isMenuOpen,
  setIsMenuOpen,
  children
}: GlobalHeaderWidgetProps) {

  return (
    <>
      <header className="sticky top-0 z-20 w-full bg-white/50 dark:bg-app-dark-blue/50 backdrop-blur-md border-b border-black/10 dark:border-white/10">
        <nav className="z-20 text-app-dark-blue dark:text-white max-w-7xl mx-auto flex items-center justify-between gap-4 px-4 py-3">
          <GlobalLogoTile width={180} height={60} className="translate-y-1" href="/" />

          {/* Right section - Custom content (e.g., Fast Navigation) */}
          <div className="flex items-center gap-3">
            {children && children}
          </div>
        </nav>
      </header>
    </>
  );
}
