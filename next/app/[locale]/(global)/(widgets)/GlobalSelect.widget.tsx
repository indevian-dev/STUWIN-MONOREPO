...
import { SelectOption } from '@stuwin/shared/types/ui/Form.types';
...
export function GlobalSelectWidget<T = string>({
    ...
    // Derive selected directly from props — no local state needed
    const selected: T | T[] = isMulti ? (value || []) : (value || "" as T)
                ...
                <div key={`dropdown-${String(value)}`} className="absolute w-full mt-1 bg-white border rounded shadow-lg z-10 max-h-60 overflow-auto">
                    ...
                    {filteredOptions.map((option) => (
                        <div
                            ...
                            className={`px-4 py-2 cursor-pointer hover:bg-app-bright-green-dark/5 flex items-center gap-2
                    ${isMulti
                                    ? isSelected(option.value) ? "bg-app-bright-green-dark/10" : ""
                                    : selected === option.value ? "bg-app-bright-green text-white" : ""
                                }`}
                            ...
                            {isMulti && (
                                <div className="w-4 h-4 border rounded flex items-center justify-center">
                                    {isSelected(option.value) && (
                                        <div className="w-2 h-2 bg-app-bright-green rounded" />
                                    )}
                                </div>
                            )}
                            {option.label}
