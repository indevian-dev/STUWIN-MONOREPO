        ...
        <div {...props}>
            {isLocalized && (
                <div className="space-y-4 mb-6">
                    <div className="flex gap-2 p-1 bg-gray-100/80 backdrop-blur-sm rounded-app w-fit border border-gray-200">
                        {SUPPORTED_LOCALES.map((locale) => (
                            <button
                                key={locale}
                                onClick={() => handleLocaleSwitch(locale)}
                                type="button"
                                className={`px-4 py-1.5 rounded-app text-sm font-medium transition-all duration-200 ${activeLocale === locale
            ...
            <div
                id={editorId}
                className="rounded-app bg-white min-h-[400px] border border-gray-200 focus-within:border-blue-500/50 focus-within:ring-4 focus-within:ring-blue-500/5 transition-all duration-300 editor-container shadow-sm"
            ...
            {isLoading && (
                <div className="text-sm text-gray-500 mt-2 flex items-center">
                    <div className="animate-spin mr-2 h-4 w-4 border-2 border-blue-500 border-t-transparent rounded-app-full"></div>
