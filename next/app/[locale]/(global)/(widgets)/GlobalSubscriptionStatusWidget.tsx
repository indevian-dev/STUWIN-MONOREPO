...
export function GlobalSubscriptionStatusWidget() {
    ...
    if (!subscriptionType || timeLeft === 'expired') {
        return (
            <div className="mx-2 mb-4 p-3 rounded-lg bg-orange-50 border border-orange-100 flex flex-col gap-2">
    ...
    return (
        <div className="mx-2 mb-4 p-3 rounded-lg bg-emerald-50 border border-emerald-100 flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-emerald-700 font-bold text-xs uppercase tracking-wider">
                    <PiCrownBold className="text-sm" />
                    {subscriptionType}
                </div>
                <div className="flex items-center gap-1 text-[10px] text-emerald-600 font-bold bg-emerald-100/50 px-2 py-0.5 rounded-full">
                    <PiClockBold />
                    {timeLeft}
                </div>
            </div>
            <div className="w-full bg-emerald-200/50 rounded-full h-1 mt-1">
                <div className="bg-emerald-500 h-1 rounded-full transition-all duration-1000" style={{ width: '75%' }}></div>
            </div>
        </div>
    );
}
