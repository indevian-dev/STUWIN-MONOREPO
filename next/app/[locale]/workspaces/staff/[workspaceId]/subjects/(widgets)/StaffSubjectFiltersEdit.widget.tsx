...
export function StaffSubjectFiltersEditWidget({ subjectId }: StaffSubjectFiltersEditWidgetProps) {
    ...
    const fetchFilters = async () => {
        try {
            const response = await fetchApiUtil<any>({ method: 'GET', url: '/api/workspaces/staff/subjects/' + subjectId + '/filters', params: {}, body: {} });
    ...
    const addFilter = async () => {
        try {
            await fetchApiUtil<any>({ method: 'POST', url: '/api/workspaces/staff/subjects/' + subjectId + '/filters/create', params: {}, body: { title: newFilterTitle, title_en: newFilterTitleEn, title_ru: newFilterTitleRu, type: filterType } });
    ...
    const deleteFilter = async (filterId: number) => {
        try {
            await fetchApiUtil<any>({ method: 'DELETE', url: '/api/workspaces/staff/subjects/' + subjectId + '/filters/' + filterId + '/delete', params: {}, body: {} });
    ...
    const addOption = async () => {
        try {
            await fetchApiUtil<any>({ method: 'POST', url: '/api/workspaces/staff/subjects/' + subjectId + '/filters/' + selectedFilterId + '/options/create', params: {}, body: { title: newOptionTitle, title_en: newOptionTitleEn, title_ru: newOptionTitleRu } });
    ...
    const deleteOption = async (optionId: number) => {
        try {
            await fetchApiUtil<any>({ method: 'DELETE', url: '/api/workspaces/staff/subjects/' + subjectId + '/filters/' + selectedFilterId + '/options/' + optionId + '/delete', params: {}, body: {} });
    ...
    return (
        <div className=' bg-gray-200 rounded-app my-4 p-4'>
            ...
            {filters.map(filter => (
                <div key={filter.id} className="mb-4 p-4 bg-white ">
                    ...
                    <ul className='grid grid-cols-1 gap-4 p-4 border border-gray-200 rounded-app'>
                        {filter.subject_filter_options.map((option: FilterOption) => (
                            <li className="bg-gray-300 text-black p-3 rounded-app" key={option.id}>
                    ...
                    {filter.type === 'STATIC' && (
                        <button type="button" onClick={() => { setSelectedFilterId(filter.id); setShowOptionModal(true); }} className="mt-2 px-4 py-2 text-white bg-green-500 rounded-app hover:bg-green-600">Add Option</button>
                    )}
                </div>
            ))}
            <button type="button" onClick={() => setShowFilterModal(true)} className="mt-2 px-4 py-2 text-white bg-blue-500 rounded-app hover:bg-blue-600">Add New Filter</button>
            ...
            {showFilterModal && (
                <Modal title="Add New Filter" onClose={() => setShowFilterModal(false)}>
                    <input
                        ...
                        className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-app"
                    />
                    <input
                        type="text"
                    ...
                    <button type='button'
                        ...
                        className="mt-2 px-4 py-2 text-white bg-blue-500 rounded-app hover:bg-blue-600"
                    >
                        Add Filter
                    </button>
                </Modal>
            ...
            {showOptionModal && (
                <Modal title="Add New Option" onClose={() => setShowOptionModal(false)}>
                    ...
                    <button type='button'
                        ...
                        className="mt-2 px-4 py-2 text-white bg-green-500 rounded-app hover:bg-green-600"
                    >
                        Add Option
                    </button>
                </Modal>
