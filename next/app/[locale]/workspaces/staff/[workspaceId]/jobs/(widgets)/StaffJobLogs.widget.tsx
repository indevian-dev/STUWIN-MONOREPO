...
import { fetchJobLogs } from '@/lib/utils/Http.StaffJobsApiHelper.util';
...
export function StaffJobLogsWidget() {
  ...
  const getStatusBadge = (status: LogStatus) => {
    ...
    return (
      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-app-full ${classes[status]}`}>
        {status}
      </span>
    );
  };
