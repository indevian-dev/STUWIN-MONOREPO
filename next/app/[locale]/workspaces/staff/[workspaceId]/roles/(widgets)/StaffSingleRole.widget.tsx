...
import {
    staffRoutes as routes,
    PERMISSIONS
} from '@/lib/routes/workspaces/staff/Staff.routes';
import { StaffSwitchButtonTile } from '@/app/[locale]/workspaces/staff/[workspaceId]/(tiles)/StaffSwitchButton.tile';
...
export function StaffSingleRoleWidget({ id }: StaffSingleRoleWidgetProps) {
    ...
    const getUniquePermissions = () => {
        ...
        Object.values(routes).forEach(route => {
            if (route.permission) {
                permissionSet.add(route.permission);
            }
        });
    ...
    if (error) return (
        <div className="p-4 bg-red-50 border border-red-200 rounded-app text-red-700 mx-4 md:mx-8">
    ...
    return (
        <div className="w-full px-4 py-4 md:px-8 md:py-8 lg:px-12 lg:py-12 space-y-6 text-black">
            {/* Role Header */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-app shadow-sm border border-gray-100">
                <div className="space-y-1">
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl font-bold text-gray-900">{role?.name}</h1>
                        <span className="px-2.5 py-0.5 rounded-app-full text-xs font-medium bg-blue-100 text-blue-800 uppercase tracking-wider">
                    ...
                    <div className="text-sm font-medium text-gray-600 flex items-center gap-2 mt-2">
                        <span className="w-2 h-2 rounded-app-full bg-green-500"></span>
            ...
            {/* Search and Controls */}
            <div className="bg-white rounded-app shadow-sm border border-gray-100 p-4 sticky top-4 z-10">
                <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-app focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-sm"
                        />
                    </div>
                    <button
                        onClick={toggleAllCategories}
                        className="px-4 py-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-app text-xs font-bold uppercase tracking-wider text-gray-600 transition-colors shadow-sm"
                    >
            ...
            <div className="space-y-4">
                {Object.entries(filteredPermissions).map(([category, categoryPermissions]) => {
                    ...
                    return (
                        <div key={category} className="bg-white rounded-app shadow-sm border border-gray-200 overflow-hidden transition-all">
                            {/* Category Header */}
                            <div
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-4">
                                        <div className={`p-2 rounded-app ${grantedCount > 0 ? 'bg-blue-100 text-blue-600' : 'bg-gray-200 text-gray-400'}`}>
            ...
            {Object.keys(filteredPermissions).length === 0 && searchTerm && (
                <div className="text-center py-20 bg-gray-50 rounded-app border-2 border-dashed border-gray-200">
                    ...
                    <button
                        ...
                        className="mt-6 px-4 py-2 bg-white border border-gray-300 rounded-app text-sm font-bold shadow-sm hover:bg-gray-50 transition-colors"
                    >
                        Clear search
                    </button>
                </div>
