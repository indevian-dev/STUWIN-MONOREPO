  ...
  return (
    <div className='w-full'>
      <Link
        ...
        className='px-6 py-2 bg-app-bright-green text-white font-semibold rounded-app hover:bg-app-bright-green-dark transition-colors'
      >
        Start New Quiz
      </Link>
      {/* Loading State */}
      ...
      {!loading && (
        <div className='space-y-4'>
          {quizzes.length === 0 ? (
            <div className='text-center py-12 bg-white rounded-app border border-gray-200'>
              ...
              <Link
                href={`/workspaces/student/${workspaceId}/quizzes/start`}
                className='inline-block mt-4 px-6 py-2 bg-app-bright-green text-white font-semibold rounded-app hover:bg-app-bright-green-dark transition-colors'
              >
                Start Quiz
              </Link>
            </div>
            ...
            quizzes.map((quiz) => (
              <div
                key={quiz.id}
                className='bg-white rounded-app shadow-md p-6 hover:shadow-lg transition-shadow'
              >
                  ...
                  <div className='flex-1'>
                    <div className='flex items-center gap-3 mb-2'>
                      ...
                      {quiz.status && typeof quiz.status === 'string' && (
                        <span className={`px-3 py-1 text-xs font-medium rounded-app-full ${getStatusBadge(quiz.status)}`}>
                  ...
                  <div className='mt-4 md:mt-0'>
                    {quiz.status === 'completed' ? (
                      <Link
                        ...
                        className='inline-block px-6 py-2 bg-app-bright-green text-white font-semibold rounded-app hover:bg-app-bright-green-dark transition-colors'
                      >
                        View Results
                      </Link>
                      ...
                      <Link
                        ...
                        className='inline-block px-6 py-2 bg-yellow-500 text-white font-semibold rounded-app hover:bg-yellow-600 transition-colors'
                      >
                        Continue
                      </Link>
      ...
      {!loading && pagination.totalPages > 1 && (
        <div className='mt-6 flex justify-center items-center space-x-2'>
          <button
            ...
            disabled={page === 1}
            className='px-4 py-2 bg-white border border-gray-300 rounded-app disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50'
          >
            Previous
          </button>
