...
import {
  apiCallForSpaHelper
} from '@/lib/utils/http/SpaApiClient';
import {
  toast
} from 'react-toastify';
...
export function StudentQuizAnalysisModal({
  ...
  const renderDigestNode = (node: any) => {
    return (
      <div key={node.id} className="relative pl-6 border-l-2 border-indigo-200 ml-4 mt-4 animate-fadeIn">
        <div className="absolute -left-2 top-0 w-4 h-4 rounded-full bg-indigo-500 border-2 border-white"></div>
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs'>
      <div className='bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col transition-all'>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="text-xs font-bold text-indigo-600 bg-indigo-50 border-none rounded-lg px-2 py-1 focus:ring-1 focus:ring-indigo-200 cursor-pointer"
              >
        ...
        <div className='flex-1 overflow-y-auto bg-gray-50/30 p-6 scroll-smooth'>
          ...
          <div className={`transition-all duration-500 ${hasStarted ? 'mb-8' : 'my-12 scale-105'}`}>
            <div className={`bg-white border rounded-xl shadow-sm p-6 relative overflow-hidden transition-colors ${selectionContext?.id === 'root-question' ? 'border-amber-400 ring-2 ring-amber-100' : 'border-blue-100'}`}>
          ...
          {!hasStarted && (
            <div className="flex flex-col items-center justify-center py-12 text-center animate-fadeIn">
              ...
              <button
                ...
                disabled={loading}
                className="group relative inline-flex items-center gap-3 px-10 py-4 bg-brand text-white font-bold text-lg rounded-full shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:scale-105 active:scale-95 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
          ...
          {selectedText && (
            <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 bg-gray-900/95 text-white backdrop-blur-lg px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 animate-slideUp z-50 border border-white/10 max-w-[90vw]">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-600/30 transition-all flex items-center gap-2 whitespace-nowrap active:scale-95 disabled:opacity-50"
              >
                {termLoading ? (
