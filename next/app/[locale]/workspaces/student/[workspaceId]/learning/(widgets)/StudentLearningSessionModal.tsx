...
export function StudentLearningSessionModal({
                        ...
                        return (
                            <div key={segmentId} className="inline-block max-w-full">
                                <div
                                    ...
                                    className={`px-1.5 py-0.5 rounded-lg cursor-pointer transition-all duration-200 prose prose-sm max-w-none inline-block ${isSelected
                                ...
                                {inlineDigest && (
                                    <div
                                        ...
                                        className="mt-1 ml-4 pl-3 border-l-2 border-indigo-200 cursor-pointer group animate-fadeIn bg-brand/10"
                                    >
    ...
    const renderDigestNode = (node: DigestNode) => {
        return (
            <div key={node.id} className="relative mt-4 first:mt-0 animate-fadeIn">
                <div className="bg-white rounded-2xl shadow-sm border border-indigo-100/50 transition-all duration-300 hover:shadow-md hover:border-indigo-200 overflow-hidden">
                    <div className="flex items-center justify-between p-4 bg-indigo-50/30 border-b border-indigo-100/30">
                        <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-sm font-black shadow-lg shadow-indigo-200">
                        ...
                        <button
                            ...
                            className="text-[10px] font-bold text-indigo-600 hover:text-white hover:bg-indigo-600 uppercase tracking-widest px-4 py-2 bg-indigo-100/50 rounded-xl transition-all active:scale-95"
                        >
    ...
    return (
        <div id="learning-modal-root" className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs'>
            <div className='bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col transition-all relative'>
                ...
                <div className='flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50 relative z-[56] shadow-sm'>
                    ...
                    <div className="flex items-center gap-4">
                        <select
                            ...
                            className="text-sm font-semibold text-indigo-600 bg-indigo-50 border-none rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-indigo-200 transition-all cursor-pointer"
                        >
                            <option value="az">Azerbaijani</option>
                            <option value="en">English</option>
                            <option value="ru">Russian</option>
                        </select>
                        <button onClick={onClose} className='p-2.5 hover:bg-gray-200 rounded-full transition-colors group'>
                ...
                >
                    ...
                    {!focusedNodeId ? (
                        ...
                        <div className={`transition-all duration-500 ${hasStarted ? 'mb-8' : 'my-12 scale-105 mx-auto max-w-2xl'}`}>
                            <div className="bg-white border rounded-2xl shadow-sm p-8 relative overflow-hidden transition-all border-indigo-100">
                                ...
                                {!hasStarted && (
                                    <div className="mt-8 pt-8 border-t border-gray-100 flex flex-col items-center gap-4 relative z-10">
                                        ...
                                        <button
                                            ...
                                            disabled={loading}
                                            className="w-full md:w-auto px-10 py-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-indigo-200 transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-3"
                                        >
                                            {loading ? (
                                                <>
                            ...
                            <div className="bg-white rounded relative overflow-hidden bg-gradient-to-br from-white to-indigo-50/20">
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <button
                                        ...
                                        disabled={termLoading}
                                        className="text-[10px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 uppercase tracking-widest px-4 py-2 rounded-xl shadow-lg shadow-indigo-200 transition-all flex items-center gap-2"
                                    >
                                        {termLoading ? (
                    ...
                    {selectedText && (
                        <div
                            className="fixed bottom-10 left-1/2 transform -translate-x-1/2 bg-gray-900/95 text-white backdrop-blur-xl px-8 py-5 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex items-center gap-8 animate-slideUp z-[70] border border-white/10 max-w-[95vw]"
                        >
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <button
                                    ...
                                    disabled={termLoading}
                                    className="px-8 py-3.5 bg-indigo-500 hover:bg-indigo-400 text-white text-base font-black rounded-2xl shadow-xl shadow-indigo-500/30 transition-all flex items-center gap-3 active:scale-95 disabled:opacity-50"
                                >
                                    {termLoading ? (
