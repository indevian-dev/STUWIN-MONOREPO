...
export function StudentLearningSessionsListWidget() {
  ...
  const fetchSessions = async () => {
    try {
      ...
      // After SpaApiCall.util: response.data = { success, data: { data: [...], pagination } }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-app shadow-sm p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-app hover:bg-primary/90 transition"
          >
            <PiPlus size={20} />
            Start New Session
          </button>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-app mb-4">
            {error}
          </div>
        )}
        ...
        {sessions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <button
              onClick={handleCreateSession}
              className="inline-flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-app hover:bg-primary/90 transition"
            >
              <PiPlus size={20} />
          ...
          <div className="space-y-3">
            {sessions.map((session) => (
              <div
                key={session.id}
                className="border border-gray-200 rounded-app p-4 hover:bg-gray-50 transition cursor-pointer"
                onClick={() =>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span
                        className={`inline-block px-2.5 py-1 rounded-app-full text-xs font-medium whitespace-nowrap ${getStatusColor(session.status)}`}
                      >
                        {session.status}
                      </span>
                    </div>
