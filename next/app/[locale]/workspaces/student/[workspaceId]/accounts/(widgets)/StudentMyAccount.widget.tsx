...
interface UserData {
    ...
    email: string;
    phone: string;
    sessions?: Record<string, { user_agent: string; last_activity: number }>;
}
...
export default function MyAccount() {
    ...
    useEffect(() => {
        async function fetchUserData() {
            try {
                setIsFetching(true);
                const response = await fetchApiUtil<MyAccountResponse>({
                ...
                if (response?.user && response?.account) {
                    ...
                    if (response.user.sessions) {
                        const sessionsObj = response.user.sessions;
                        const sessionsArray = Object.keys(sessionsObj).map(sessionId => ({
                            id: sessionId,
                            ...sessionsObj[sessionId],
                            isCurrentSession: response.currentSession === sessionId
                        }));
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await fetchApiUtil<any>({
                method: 'PATCH',
                url: '/api/workspaces/dashboard/accounts/me/update',
                body: JSON.stringify({
                    user: {
                        name: user.name,
    ...
    const handleLogoutSession = async (sessionId: string) => {
        try {
            ...
            await fetchApiUtil<any>({
                method: 'POST',
                url: '/api/workspaces/dashboard/accounts/me/logout-session',
                body: JSON.stringify({
                    sessionId
                })
            });
    ...
    return (
        <div className="bg-white rounded-app shadow-md px-4 md:px-8 w-full mx-auto">
            <div className="flex justify-between items-center py-6 border-b border-gray-200">
                ...
                {!isEditMode ? (
                    <button
                        ...
                        className="px-4 py-2 bg-bd text-white rounded-app hover:bg-bd/90 transition-colors"
                    >
                        Edit Profile
                    </button>
                ) : null}
            ...
            {error && (
                <div className="bg-rose-50 text-rose-500 p-4 rounded-app my-4">
                    {error}
                </div>
            )}
                    ...
                    <div className="space-y-6">
                        <div className="flex flex-col md:flex-row gap-6">
                            <div className="w-full md:w-1/4">
                                <div className="aspect-square rounded-app-full bg-black/5 dark:bg-white/5 flex items-center justify-center overflow-hidden relative">
                                    <GlobalImagePlaceholderTile aspect="" className="absolute inset-0 h-full w-full rounded-app-full" />
                                    <img
                                        src={`${process.env.NEXT_PUBLIC_S3_PREFIX}${user.id}/avatar/avatar.webp`}
                                        alt={`${user.name}'s avatar`}
                                        className="absolute inset-0 w-full h-full object-cover rounded-app-full z-10"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-lg font-medium">
                                        <span className="inline-block px-3 py-1 bg-bl text-bd rounded-app-full">
                                                        ...
                                                        <div className="flex items-center">
                                                            <div className="text-sm font-medium text-gray-900">
                                                                ...
                                                                {session.isCurrentSession &&
                                                                    <span className="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-app-full text-xs font-medium bg-emerald-100 text-emerald-800">
                                                                        Current
                                                                    </span>
                                                                }
                                                            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="mb-6">
                            ...
                            <div className="flex items-center space-x-6">
                                <div className="w-24 h-24 rounded-app-full bg-black/5 dark:bg-white/5 flex items-center justify-center overflow-hidden relative">
                                    <GlobalImagePlaceholderTile aspect="" className="absolute inset-0 h-full w-full rounded-app-full" />
                                    <img
                                        ...
                                        className="absolute inset-0 w-full h-full object-cover rounded-app-full z-10"
                                        onLoad={(e) => { (e.currentTarget as HTMLImageElement).style.opacity = '1'; }}
                                        onError={(e) => { if (!avatarPreview) (e.currentTarget as HTMLImageElement).style.display = 'none'; }}
                                        style={{ opacity: avatarPreview ? 1 : 0, transition: 'opacity 0.3s' }}
                                    />
                                </div>
                                <div>
                                    <label className="cursor-pointer bg-bl text-bd px-4 py-2 rounded-app hover:bg-bl/90 transition-colors">
                                        Upload New Photo
                                        <input
                                            type="file"
                                            accept="image/*"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className="mt-1 block w-full p-2 rounded-app border border-gray-300 shadow-sm focus:border-bl focus:ring-bl"
                                />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    value={user.email || ''}
                                    disabled
                                    className="mt-1 block w-full p-2 rounded-app border border-gray-300 bg-gray-100 shadow-sm"
                                />
                        ...
                        <div className="pt-6 flex justify-end space-x-4">
                            <button
                                ...
                                className="px-4 py-2 bg-gray-200 text-gray-700 rounded-app hover:bg-gray-300 transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="px-4 py-2 bg-bd text-white rounded-app hover:bg-bd/90 transition-colors disabled:opacity-50"
                            >
