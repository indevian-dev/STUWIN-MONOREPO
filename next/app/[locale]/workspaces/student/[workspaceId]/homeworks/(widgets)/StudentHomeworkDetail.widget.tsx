...
import { StudentHomeworkAiSidebarWidget } from "./StudentHomeworkAiSidebar.widget";
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2 mb-4">
          <button
            onClick={() => router.back()}
            className="p-2 hover:bg-gray-100 rounded-app transition"
          >
            <PiArrowLeft size={20} />
          </button>
        </div>
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-app">
      ...
      <div className={`flex-1 overflow-y-auto transition-all duration-300 ${isAiOpen ? 'pr-0' : 'pr-0'}`}>
        <div className="p-6 space-y-6 max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={() => router.back()}
              className="p-2 hover:bg-gray-100 rounded-app transition flex items-center gap-2 text-gray-600"
              title="Go back"
            >
              <PiArrowLeft size={20} />
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-app font-medium transition-all ${isAiOpen
                ? 'bg-app-bright-green text-white shadow-lg'
                : 'bg-white border border-gray-200 text-gray-700 hover:border-app hover:text-app-bright-green'
                }`}
            >
              <PiRobot size={20} />
          ...
          <div className="bg-white rounded-app border border-gray-100 shadow-sm p-8 space-y-8">
            {/* Header Section */}
            <div className="flex justify-between items-start pb-6 border-b border-gray-100">
              <div className="flex-1">
                ...
                <div className="flex gap-4 flex-wrap">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`inline-block px-3 py-1 rounded-app-full text-xs font-semibold ${getStatusColor(homework.status)}`}
                    >
                      {homework.status}
                    </span>
                  </div>
                  {homework.grade !== undefined && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-gray-900 bg-green-50 px-3 py-1 rounded-app-full border border-green-100">
                        {homework.grade}/100
                      </span>
                    </div>
                  )}
              ...
              <div className="flex gap-2">
                {homework.status !== "graded" && (
                  <button
                    onClick={handleEdit}
                    className="p-2 text-gray-400 hover:text-app-bright-green hover:bg-app-bright-green/5 rounded-app transition"
                    title="Edit"
                  >
                    <PiPencil size={22} />
                  </button>
                ...
                <button
                  ...
                  className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-app transition"
                  title="Delete"
                >
                  <PiTrash size={22} />
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-8">
                ...
                {homework.textContent && (
                  <div>
                    ...
                    <div className="bg-gray-50/50 border border-gray-100 p-6 rounded-app">
              ...
              <div className="space-y-6">
                {/* Dates & Timeline */}
                <div className="bg-gray-50/50 rounded-app p-6 border border-gray-100 space-y-4">
                ...
                {homework.feedback && homework.status === "graded" && (
                  <div className="bg-app-bright-green/5 border border-app/10 p-6 rounded-app space-y-3">
                    <h2 className="text-xs font-bold text-app-bright-green uppercase tracking-widest">Teacher's Feedback</h2>
                    <p className="text-app-dark-blue dark:text-white leading-relaxed text-sm font-medium">
                      {homework.feedback}
                    </p>
                  </div>
                )}
              ...
              <div className="pt-8 border-t border-gray-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {homework.images.map((image) => (
                    <div key={image.id} className="relative group overflow-hidden rounded-app border border-gray-100 shadow-sm aspect-video">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                        <a
                          href={image.url}
                          download
                          className="p-3 bg-white rounded-app-full shadow-xl hover:bg-gray-50 transition"
                        >
                          <PiDownload size={22} className="text-app-bright-green" />
                        </a>
                      </div>
                    </div>
                  ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between pt-8 border-t border-gray-100">
              <button
                ...
                className="px-6 py-3 border border-gray-200 text-gray-600 rounded-app hover:bg-gray-50 transition font-bold text-sm"
              >
                Return to List
              </button>
              {homework.status === "pending" && (
                <button
                  ...
                  className="px-8 py-3 bg-app-bright-green text-white rounded-app hover:bg-app-bright-green/90 hover:shadow-lg transition-all font-bold text-sm"
                >
                  Turn in Homework
                </button>
              )}
