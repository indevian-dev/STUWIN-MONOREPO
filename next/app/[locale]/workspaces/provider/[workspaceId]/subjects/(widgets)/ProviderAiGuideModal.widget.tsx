...
export function ProviderAiGuideModalWidget({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            {/* Modal */}
            <div className="relative bg-white rounded-app shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                        <div className="p-2 rounded-app bg-amber-50">
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 rounded-app hover:bg-gray-100 transition-colors"
                    >
                ...
                <div className="px-6 py-4 space-y-4">
                    ...
                    {error && (
                        <div className="bg-red-50 border border-red-200 rounded-app p-3 text-red-700 text-sm">
                            {error}
                        </div>
                    )}
                    ...
                    <textarea
                        ...
                        rows={8}
                        className="w-full px-4 py-3 border border-gray-200 rounded-app text-sm font-mono focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 transition-colors resize-y"
                ...
                <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                    <button
                        onClick={onClose}
                        disabled={saving}
                        className="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-app hover:bg-gray-50 transition-colors disabled:opacity-50"
                    >
                        {t("cancel")}
                    </button>
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-amber-600 rounded-app hover:bg-amber-700 transition-colors disabled:opacity-50"
                    >
