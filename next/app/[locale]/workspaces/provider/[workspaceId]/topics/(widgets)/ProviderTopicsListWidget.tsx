...
export function ProviderTopicsListWidget() {
  ...
  }>({ isOpen: false, topicId: "", subjectId: "", currentCrib: null });
              ...
              <button
                onClick={() =>
                  setCribModalState({
                    ...
                    currentCrib: topic.aiAssistantCrib || null,
                  })
                }
                className={`px-3 py-1.5 border rounded flex items-center gap-1.5 text-sm font-medium transition-colors ${topic.aiAssistantCrib
                  ? "border-amber-400 bg-amber-50 hover:bg-amber-100 text-amber-700"
                  : "border-gray-300 bg-white hover:bg-gray-50 text-gray-500 hover:text-gray-700"
                  }`}
                title="AI Assistant Crib"
              >
                <PiBrain className={`w-4 h-4 ${topic.aiAssistantCrib ? "fill-current" : ""}`} />
                Crib
              </button>
              <button
                onClick={() => {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <ProviderCribModalWidget
        ...
        entityType="topic"
        entityId={cribModalState.topicId}
        subjectId={cribModalState.subjectId}
        currentCrib={cribModalState.currentCrib}
        onClose={() => setCribModalState((s) => ({ ...s, isOpen: false }))}
        onSuccess={handleTopicSuccess}
      />
    </div>
  );
