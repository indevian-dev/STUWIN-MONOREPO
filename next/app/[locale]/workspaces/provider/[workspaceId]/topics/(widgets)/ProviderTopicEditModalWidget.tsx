...
export function ProviderTopicEditModalWidget({
  ...
  // New Crib State
  const [aiAssistantCrib, setAiAssistantCrib] = useState<string>("");
  ...
  const fetchTopicData = async (): Promise<void> => {
    ...
    try {
      ...
      if (response?.data?.topic) {
        ...
        setAiAssistantCrib(topic.aiAssistantCrib || "");
      }
  ...
  const handleSubmit = async (
    ...
    try {
      const response = await apiCallForSpaHelper({
        method: "PUT",
        url: `/api/workspaces/provider/${workspaceId}/topics/${topicId}/update`,
        params: {},
        body: {
          name,
          ...(body && { body }),
          ...
          ...(gradeLevel && { grade_level: parseInt(gradeLevel) }),
          ...(subjectId && { subject_id: subjectId }),
          ...(aiAssistantCrib && { ai_assistant_crib: aiAssistantCrib }),
        },
      });
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label
                      ...
                      className="block text-sm font-medium text-gray-700 mb-2"
                    >
                      AI Assistant Crib (Hidden Instructions)
                    </label>
                    <textarea
                      id="ai-crib"
                      value={aiAssistantCrib}
                      onChange={(e) => setAiAssistantCrib(e.target.value)}
                      rows={3}
                      disabled={loading}
                      className="w-full px-3 py-2 border border-blue-200 bg-blue-50/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm disabled:bg-gray-50 disabled:opacity-50"
                      placeholder="Enter instructions for AI when using this topic... (e.g., 'Focus on mental math strategies')"
                    />
                  </div>
