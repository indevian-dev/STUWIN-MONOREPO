...
export function ProviderOrganizationWidget({ organization }: ProviderOrganizationWidgetProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Status banner */}
      <div className={`flex items-center gap-3 px-4 py-3 rounded-app ${organization.isActive ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
        }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-app shadow-sm border border-neutral-200 p-6">
          <h2 className="text-lg font-semibold text-app-dark-blue dark:text-white flex items-center gap-2 mb-4 pb-3 border-b border-neutral-100">
            <PiBuilding className="text-app-bright-green text-xl" />
            {t('basic_information')}
          </h2>
            ...
            {organization.logo && (
              <div className="flex justify-center mb-4">
                <img
                  ...
                  className="w-24 h-24 object-contain rounded-app border border-neutral-200"
                />
              </div>
            )}
        ...
        <div className="bg-white rounded-app shadow-sm border border-neutral-200 p-6">
          <h2 className="text-lg font-semibold text-app-dark-blue dark:text-white flex items-center gap-2 mb-4 pb-3 border-b border-neutral-100">
            <PiPhone className="text-app-bright-green text-xl" />
        ...
        <div className="bg-white rounded-app shadow-sm border border-neutral-200 p-6">
          <h2 className="text-lg font-semibold text-app-dark-blue dark:text-white flex items-center gap-2 mb-4 pb-3 border-b border-neutral-100">
            <PiCurrencyDollar className="text-app-bright-green text-xl" />
        ...
        <div className="bg-white rounded-app shadow-sm border border-neutral-200 p-6">
          <h2 className="text-lg font-semibold text-app-dark-blue dark:text-white flex items-center gap-2 mb-4 pb-3 border-b border-neutral-100">
            <PiMapPin className="text-app-bright-green text-xl" />
      ...
      {m?.features && (m.features as string[]).length > 0 && (
        <div className="bg-white rounded-app shadow-sm border border-neutral-200 p-6">
          <h2 className="text-lg font-semibold text-app-dark-blue dark:text-white flex items-center gap-2 mb-4 pb-3 border-b border-neutral-100">
            <PiStar className="text-app-bright-green text-xl" />
            {t('features')}
          </h2>
          <div className="flex flex-wrap gap-2">
            {(m.features as string[]).map((feature, idx) => (
              <span
                key={idx}
                className="inline-flex items-center px-3 py-1.5 rounded-app-full text-sm bg-app-bright-green/5 text-app-bright-green border border-app/10"
              >
                {feature}
              </span>
            ))}
...
function InfoRow({ label, value, multiline }: { label: string; value: string; multiline?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-app-dark-blue dark:text-white mt-0.5 ${multiline ? 'whitespace-pre-line' : 'font-medium'}`}>{value}</p>
    </div>
  );
}
        ...
        {value ? (
          isLink ? (
            <a
              ...
              className="block text-app-bright-green hover:underline mt-0.5"
            >
              {value}
            </a>
          ) : (
            <p className="text-app-dark-blue dark:text-white font-medium mt-0.5">{value}</p>
          )
        ) : (
