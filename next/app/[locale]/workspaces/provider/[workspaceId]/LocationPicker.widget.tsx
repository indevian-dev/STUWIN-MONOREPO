...
interface StudentLocationPickerWidgetProps {
  isOpen: boolean;
  onClose: () => void;
  onLocationSelect: (location: LocationCoordinates) => void;
}

export function StudentLocationPickerWidget({ isOpen, onClose, onLocationSelect }: StudentLocationPickerWidgetProps) {
    ...
    const initializeMap = async () => {
      ...
      try {
        ...
        newMap.on('click', (e) => {
          ...
          const markerElement = container.querySelector('.map-marker') as HTMLElement;
          if (markerElement) {
  ...
  if (error) {
    return (
      <div className={`${isOpen ? 'fixed' : 'hidden'} inset-0 z-20 bg-slate-950 bg-opacity-90 overflow-y-auto w-full m-auto flex items-center justify-center`}>
        <div className="w-full max-w-md shadow-xl rounded-app bg-white z-100 relative p-6">
  ...
  return (
    <div className={`${isOpen ? 'fixed' : 'hidden'} inset-0 z-20 bg-slate-950 bg-opacity-90 overflow-y-auto w-full m-auto flex items-center justify-center`}>
      <div className="w-full h-full lg:h-auto lg:w-1/2 lg:aspect-video shadow-xl lg:rounded-app bg-white z-100 relative">
        <div
          ref={mapContainerRef}
