    ...
    return (
        <section className='relative m-auto max-w-screen-xl grid gap-3 grid-cols-1 lg:grid-cols-3 justify-center px-4 py-20 '>
            {subjects.map((subject) => (
                <div
                    key={subject.id}
                    className='w-full cursor-pointer hover:bg-gray-100 rounded-md transition-colors duration-200 relative aspect-[5/2]'
                    onClick={() => handleSubjectClick(subject.id, subject.slug)}
                >
                    <Image src={`${process.env.NEXT_PUBLIC_S3_PREFIX || ''}/subjects/${subject.cover}`} alt={subject.title || 'Subject'} fill objectFit='cover' className='rounded-md object-cover opacity-80' />
                    <h2 className=' absolute bottom-8 left-8 right-8 text-4xl font-black p-2 line-clamp-1 text-dark'>{subject.title || 'Subject'}</h2>
                </div>
            ))}
        </section>
    );
