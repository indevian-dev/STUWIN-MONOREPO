...
interface StaffImagesGalleryWidgetProps {
  images: string[];
...
export default function StaffImagesGalleryWidget({ 
  ...
  return (
    <div className="flex flex-col items-center w-full md:max-w-[200px] lg:max-w-[300px] pl-4">
      ...
      {images.length > 1 && (
        <div>
          <button onClick={prevImage} className="mr-2 bg-brand text-dark hover:bg-brand/80 font-bold py-2 px-3 rounded-full">←</button>
          <button onClick={nextImage} className=' bg-brand text-dark hover:bg-brand/80 font-bold py-2 px-3 rounded-full'>→</button>
        </div>
      )}
    </div>
  );
